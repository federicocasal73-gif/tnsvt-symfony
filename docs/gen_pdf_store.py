...
================================================
Genera un PDF profesional desde docs/play-store.md
usando reportlab Platypus.

Uso:
    py docs/gen_pdf.py [output_path]

Si no se pasa output_path, usa docs/play-store.pdf
"""
import re
import sys
import os
...
if __name__ == '__main__':
    ...
    md_path = os.path.join(script_dir, 'play-store.md')

    if len(sys.argv) >= 2:
        pdf_path = sys.argv[1]
    else:
        pdf_path = os.path.join(script_dir, 'play-store.pdf')
